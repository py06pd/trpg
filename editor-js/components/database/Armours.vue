<template>
    <div>
        <ul class="cic-container">
            <li class="cic-header">
                <span class="cic-header-title">{{ $t('Armours') }}</span>
                <span
                    id="add_button"
                    :title="$t('Add Armour')"
            ...
            <li>
                ...
                <table id="armours_table" class="cic-table">
                    <thead>
                        <tr>
                            <th></th>
                            <th>{{ $t('Name') }}</th>
                    ...
                    <tbody>
                        <tr
                            v-for="(item, index) in game.getArmours().filter(x => !filter || filter === x.getGearType())"
                            :key="index"
...
<script>
    ...
    import EditArmour from '@/components/database/EditArmour';
    ...
    export default {
        ...
        methods: {
            deleteItems () {
                this.game.removeArmours(this.selected);
                this.selected = [];
            },
            ...
            openPopup (itemId, evt) {
                if (itemId === -1 || evt.target.classList.length === 0 || !evt.target.classList.contains('checkbox')) {
                    if (itemId !== -1) {
                        this.selectedItem = this.game.getArmours().find(x => x.getId() === itemId);
                    } else {
                        this.selectedItem = new ItemModel();
                        this.game.addArmour(this.selectedItem);
                    }
                }
            },
        },
