<template>
    <div>
        <ul class="cic-container">
            <li class="cic-header">
                <span class="cic-header-title">{{ $t('Items') }}</span>
                <span
                    id="add_button"
                    :title="$t('Add Item')"
            ...
            <li>
                <table id="items_table" class="cic-table">
                    <thead>
                        <tr>
                            <th></th>
                            <th>{{ $t('Name') }}</th>
                    ...
                    <tbody>
                        <tr
                            v-for="(item, index) in game.getItems()"
                            :key="index"
...
<script>
    ...
    import EditItem from '@/components/database/EditItem';
    ...
    export default {
        ...
        methods: {
            deleteItems () {
                this.game.removeItems(this.selected);
                this.selected = [];
            },
            ...
            openPopup (itemId, evt) {
                if (itemId === -1 || evt.target.classList.length === 0 || !evt.target.classList.contains('checkbox')) {
                    if (itemId !== -1) {
                        this.selectedItem = this.game.getItems().find(x => x.getId() === itemId);
                    } else {
